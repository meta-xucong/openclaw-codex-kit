...
- **只能使用 Python 调用本技能**，必须通过 `python`/`python` 执行本技能提供的脚本。
...
- API key 只从环境变量 `IMAGE_2_API_KEY` 或 `OPENAI_API_KEY` 读取；服务端点由 `IMAGE_2_API_BASE_URL` 指定。
...
- 确认清单默认写入 `./codex-data/workspace/createContent/image/.confirmations/`，可用 `--confirmation-output` 覆盖；清单文件权限 600。
