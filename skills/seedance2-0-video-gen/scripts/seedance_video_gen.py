...
# 从用户目录下的配置文件加载配置。准备确认清单和 dry-run 不需要 API Key，
# 因此配置错误不应在模块导入阶段直接退出。
sosoagent_json_path = Path.home() / ".sosoagent" / "sosoclaw.json"
API_KEY = None
API_KEY_LOAD_ERROR = None

if not sosoagent_json_path.exists():
    API_KEY_LOAD_ERROR = f"{sosoagent_json_path} not found"
else:
    try:
        with open(sosoagent_json_path, "r", encoding="utf-8") as f:
            env_data = json.load(f)
            API_KEY = env_data.get("API_KEY")
        if not API_KEY:
            API_KEY_LOAD_ERROR = f"API_KEY not found in {sosoagent_json_path}"
    except json.JSONDecodeError as e:
        API_KEY_LOAD_ERROR = f"Invalid JSON in {sosoagent_json_path}: {str(e)}"
    except Exception as e:
        API_KEY_LOAD_ERROR = f"Failed to read {sosoagent_json_path}: {str(e)}"
...
DEFAULT_API_BASE_URL = "https://sosoagent.com"
BASE_URL = f"{DEFAULT_API_BASE_URL}/api/llm/doubao/contents/generations/tasks"
...
def api_key_error_result():
    return error_result(
        "ConfigError",
        API_KEY_LOAD_ERROR or f"API_KEY not found in {sosoagent_json_path}",
        "Configuration",
    )
...
def resolve_video_output_dir(agent_id=None, output_dir=None):
    ...
    if normalized_agent_id:
        if normalized_agent_id.lower() == "main":
            return (Path.home() / ".sosoagent" / "workspace" / "createContent" / "video").resolve()
        return (
            Path.home() / ".sosoagent" / "workspaces" / normalized_agent_id / "createContent" / "video"
        ).resolve()
...
def persist_video_to_local(video_url, agent_id=None, output_dir=None, file_stem="seedance-video", task_id=None):
    ...
    request = urllib.request.Request(
        video_url,
        headers={"Accept": "*/*", "User-Agent": "sosoagent-seedance-video-gen/1.0"},
        method="GET",
    )
...
def get_session_file_path(agent_id):
    """
    根据 agent_id 获取最近的会话文件路径

    读取 ~/.sosoagent/agents/{agent_id}/sessions/sessions.json
    从 agent:{agent_id}:main 对象中获取 sessionFile 字段
    返回 sessionFile 指向的 jsonl 文件路径
    """
    sessions_json_path = Path.home() / ".sosoagent" / "agents" / agent_id / "sessions" / "sessions.json"

    if not sessions_json_path.exists():
        raise RuntimeError(f"Sessions 文件不存在: {sessions_json_path}")

    try:
        with open(sessions_json_path, "r", encoding="utf-8") as f:
            sessions_data = json.load(f)
    except Exception as e:
        raise RuntimeError(f"读取 sessions.json 失败: {str(e)}")

    # 获取 agent:{agent_id}:main 对象
    agent_key = f"agent:{agent_id}:main"
    agent_data = sessions_data.get(agent_key)
    if not agent_data:
        raise RuntimeError(f"sessions.json 中没有 {agent_key} 对象")

    # 从 agent 对象中获取 sessionFile 路径
    session_file = agent_data.get("sessionFile")
    if not session_file:
        raise RuntimeError(f"{agent_key} 中没有 sessionFile 字段")

    session_file_path = Path(session_file)
    if not session_file_path.exists():
        raise RuntimeError(f"会话文件不存在: {session_file_path}")
...
def extract_media_urls_from_content(content, media_type):
    """兼容 OpenClaw 会话中 string/list/dict 形式的图片、视频和音频内容。
...
def collect_media_for_confirmation(args, media_type):
    configs = {
        "image": {
            ...
            "extractor": extract_images_from_session, "label": "图片",
        },
        "video": {
            "plural": "videos", "expected": "expected_video_count",
            "extractor": extract_videos_from_session, "label": "视频",
        },
        "audio": {
            "plural": "audios", "expected": "expected_audio_count",
            "extractor": extract_audios_from_session, "label": "音频",
        },
    }
    ...
    if expected_count > 0:
        try:
            session_file = get_session_file_path(args.agent_id)
            session_urls = dedupe_media_urls(config["extractor"](session_file))
        except Exception as error:
            session_error = error

    # 会话消息中的完整 URL 是权威来源：禁止智能体拆分 attachment id 与文件名后重组。
    if expected_count > 0 and len(session_urls) == expected_count:
        media_urls = session_urls
        if cli_urls and not media_url_lists_match(cli_urls, session_urls):
            media_source = f"latest_user_message_with_{plural}_overriding_cli"
            return {
                "success": True,
                plural: media_urls,
                f"{media_type}_source": media_source,
                "warning": (
                    f"命令行 --{media_type} 与用户消息中的参考{config['label']} URL 不一致"
                    f"（常见于拆分 attachment id 与文件名后错误重组）。"
                    f"已忽略命令行，改用会话消息中的完整 URL 与上传顺序。"
                ),
                f"cli_{plural}": cli_urls,
                f"session_{plural}": session_urls,
            }
        media_source = (
            f"latest_user_message_with_{plural}"
            if not cli_urls
            else f"latest_user_message_with_{plural}_matching_cli"
        )
    elif cli_urls:
        media_urls = cli_urls
        media_source = "explicit_cli"
    elif expected_count > 0:
        if session_error is not None:
            return error_result(
                f"Session{media_type.title()}ExtractionFailed",
                f"从会话提取参考{config['label']}失败: {session_error}",
                "IOError",
            )
        media_urls = session_urls
        media_source = f"latest_user_message_with_{plural}"
    else:
        media_urls = []
        media_source = f"confirmed_no_{plural}"
