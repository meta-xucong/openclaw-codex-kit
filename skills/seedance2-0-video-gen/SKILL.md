...
Windows 通常使用 `python`。使用脚本绝对路径，不依赖 `~` 展开，也不使用旧版 Windows PowerShell 不支持的 `&&`。
    ...
    python "C:\Users\<User>\.sosoagent\workspaces\<AgentID>\skills\seedance2-0-video-gen\scripts\seedance_video_gen.py" --help

macOS / Linux 首选：

    python3 --version

若不存在，再检查 `python --version`。macOS 通常使用 `python3`。

    python3 "$HOME/.sosoagent/workspaces/<AgentID>/skills/seedance2-0-video-gen/scripts/seedance_video_gen.py" --help
...
Windows：
    ...
    python "C:\Users\<User>\.sosoagent\workspaces\<AgentID>\skills\seedance2-0-video-gen\scripts\seedance_video_gen.py" "<完整最终提示词>" --agent-id "<AgentID>" --duration 11 --expected-image-count 2 --image "<图片1 URL>" --image "<图片2 URL>" --expected-video-count 1 --video "<视频1 URL>" --expected-audio-count 1 --audio "<音频1 URL>" --prepare-confirmation

macOS / Linux：

    python3 "$HOME/.sosoagent/workspaces/<AgentID>/skills/seedance2-0-video-gen/scripts/seedance_video_gen.py" "<完整最终提示词>" --agent-id "<AgentID>" --duration 11 --expected-image-count 2 --image "<图片1 URL>" --image "<图片2 URL>" --expected-video-count 1 --video "<视频1 URL>" --expected-audio-count 1 --audio "<音频1 URL>" --prepare-confirmation
...
Windows：
    ...
    python "C:\Users\<User>\.sosoagent\workspaces\<AgentID>\skills\seedance2-0-video-gen\scripts\seedance_video_gen.py" --agent-id "<AgentID>" --confirmation-file "<清单绝对路径>" --confirm-fingerprint "<完整指纹>" --dry-run

macOS / Linux：

    python3 "$HOME/.sosoagent/workspaces/<AgentID>/skills/seedance2-0-video-gen/scripts/seedance_video_gen.py" --agent-id "<AgentID>" --confirmation-file "<清单绝对路径>" --confirm-fingerprint "<完整指纹>" --dry-run
...
Windows：
    ...
    python "C:\Users\<User>\.sosoagent\workspaces\<AgentID>\skills\seedance2-0-video-gen\scripts\seedance_video_gen.py" --agent-id "<AgentID>" --confirmation-file "<清单绝对路径>" --confirm-fingerprint "<完整指纹>" --confirm-charge --user-confirmation "确认"

macOS / Linux：

    python3 "$HOME/.sosoagent/workspaces/<AgentID>/skills/seedance2-0-video-gen/scripts/seedance_video_gen.py" --agent-id "<AgentID>" --confirmation-file "<清单绝对路径>" --confirm-fingerprint "<完整指纹>" --confirm-charge --user-confirmation "确认"

OpenClaw exec 使用脚本绝对路径，timeout 不低于 2400 秒，yieldMs 不低于 2400000。进度写入 stderr，最终 JSON 写入 stdout。
...
| 情况 | 允许操作 | 禁止操作 |
|---|---|---|
| Python 或脚本尚未启动 | 修正 python/python3/py -3、绝对路径、shell 引号；运行 `--help` 或同一清单的 dry-run | 使用 test prompt；改动或删除媒体；创建试跑业务输入 |
...
恢复未知提交：
    ...
    python3 "$HOME/.sosoagent/workspaces/<AgentID>/skills/seedance2-0-video-gen/scripts/seedance_video_gen.py" --agent-id "<AgentID>" --recover-submission "<submission_key>"

本地产物对账（只读远端、只更新本地 JSON，不发起 POST）：

    python3 "$HOME/.sosoagent/workspaces/<AgentID>/skills/seedance2-0-video-gen/scripts/seedance_video_gen.py" --agent-id "<AgentID>" --reconcile-artifacts

对账结果中的 `untracked_artifacts` 表示目录中存在但尚未能唯一关联到 task_id 的旧视频；不能凭文件名猜测归属，应先人工确认后再补写任务记录。

查询指定历史任务：

    python3 "$HOME/.sosoagent/workspaces/<AgentID>/skills/seedance2-0-video-gen/scripts/seedance_video_gen.py" --agent-id "<AgentID>" --query-history --task-id "<task_id>"
...
- main：`~/.sosoagent/workspace/createContent/video/`
- 其他智能体：`~/.sosoagent/workspaces/<AgentID>/createContent/video/`
