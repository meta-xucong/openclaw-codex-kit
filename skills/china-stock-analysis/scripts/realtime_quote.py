...
数据源：新浪财经（统一接口）
支持：沪市(sh)、深市(sz)、北交所(bj) 股票

Usage:
    python scripts/realtime_quote.py 600519              # 单只股票
    python scripts/realtime_quote.py 600519 000858       # 多只股票
    python scripts/realtime_quote.py 600519 --minute     # 分时量能分析
    python scripts/realtime_quote.py 600519 --json       # JSON输出
"""
...
def fetch_realtime_sina(symbols: list[str]) -> dict[str, dict]:
    ...
    新浪接口返回格式:
    var hq_str_sh600519="名称,今开,昨收,现价,最高,最低,买一,卖一,成交量(股),成交额(元),...";
