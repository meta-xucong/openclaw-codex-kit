...
SOSOCLAW_JSON_PATH = Path.home() / ".sosoagent" / "sosoclaw.json"
IMAGE_2_API_BASE_URL = str(
    os.environ.get("IMAGE_2_API_BASE_URL") or "https://sosoagent.com"
).strip().rstrip("/")
...
def load_runtime_config():
    if SOSOCLAW_JSON_PATH.exists():
        try:
            with open(SOSOCLAW_JSON_PATH, "r", encoding="utf-8") as file:
...
API_KEY = str(
    ...
    or os.environ.get("SOSOCLAW_API_KEY")
...
def build_multipart_form(fields, files):
    boundary = f"----sosoclaw-image-2-{uuid.uuid4().hex}"
    parts = []
...
def generate_image(prompt, size="2048x2048", model=DEFAULT_MODEL, agent_id=None):
    if not API_KEY:
        return {"error": f"API key not found. Set IMAGE_2_API_KEY or API_KEY in {SOSOCLAW_JSON_PATH}"}
...
CONFIRMATION_DIR = Path.home() / ".sosoagent" / "workspace" / "createContent" / "image" / ".confirmations"
