...
```bash
# 查询航班信息
python3 skills/web-search-extraction/scripts/web_search.py "[出发地]到[目的地] [日期] 机票 航班" --json

# 查询机票价格
python3 skills/web-search-extraction/scripts/web_search.py "[出发地]到[目的地] 机票价格  cheapest" --json

# 查询航空公司
python3 skills/web-search-extraction/scripts/web_search.py "[出发地]到[目的地] 直飞航班 航空公司" --json
```

**示例：**

```bash
python3 skills/web-search-extraction/scripts/web_search.py "北京到成都 6月15日 机票 价格" --json
```
...
```bash
# 签证和入境要求
python3 skills/web-search-extraction/scripts/web_search.py "[目的地] 签证要求 中国公民 2024" --json

# 最佳旅行时间
python3 skills/web-search-extraction/scripts/web_search.py "[目的地] 最佳旅游时间 天气 季节" --json

# 必去景点
python3 skills/web-search-extraction/scripts/web_search.py "[目的地] 必去景点 旅游攻略" --json

# 美食推荐
python3 skills/web-search-extraction/scripts/web_search.py "[目的地] 特色美食 餐厅推荐" --json

# 住宿区域
python3 skills/web-search-extraction/scripts/web_search.py "[目的地] 住哪里方便 酒店推荐 区域" --json

# 交通指南
python3 skills/web-search-extraction/scripts/web_search.py "[目的地] 交通攻略 地铁 公交 打车" --json

# 避坑指南
python3 skills/web-search-extraction/scripts/web_search.py "[目的地] 旅游避坑 注意事项 骗局" --json

# 行程参考
python3 skills/web-search-extraction/scripts/web_search.py "[目的地] [天数]天行程 旅游攻略" --json
```
...
**Data Storage:**
数据存储在用户主目录下的 `.claude/travel_planner/` 文件夹中：
- Preferences: `preferences.json`
- Trips: `trips.json`

*路径示例：*
- macOS/Linux: `~/.claude/travel_planner/`
- Windows: `%USERPROFILE%\.claude\travel_planner\`
