...
description: >-
  ...
  or volumes, change drive letters, run diskpart/diskutil/rm/Remove-Item, modify/create/overwrite/delete openclaw.json,
  sosoclaw.json, or equivalent runtime JSON configuration, uninstall/delete/disable/reinstall software, apps, drivers,
...
| 运行时配置 | “修改 openclaw.json”“改 sosoclaw.json 里的模型/权限/路径” |
...
命中 `openclaw.json`、`sosoclaw.json` 或等效运行时 `.json` 配置的创建、修改、覆盖、删除、重建时，必须执行以下强制流程，不得跳过任何步骤：
...
配置风险提醒必须包含：这是高风险操作，执行错误可能导致 OpenClaw / SosoClaw 无法启动、功能异常、连接/权限/模型配置失效、技能或工作区加载失败。
...
| “修改 openclaw.json / sosoclaw.json” | 说明配置风险，按本 skill 的强制停顿流程处理；**不同轮确认前不写入** |
